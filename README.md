...
This is inspired by [node-jsontoxml](https://github.com/soldair/node-jsontoxml), which was found to be a bit too rough around the edges. @wemnyelezxnpm/expedita-dolor-quibusdam attempts to fix that by being more flexible.

### Installation

-   npm install @wemnyelezxnpm/expedita-dolor-quibusdam

### Simple example

```js
import { toXML } from '@wemnyelezxnpm/expedita-dolor-quibusdam';
...
-   Initial support for XML comments ([#47](https://github.com/wemnyelezxnpm/expedita-dolor-quibusdam/issues/47))

#### Version 2.1.1

-   Fix for [#48](https://github.com/wemnyelezxnpm/expedita-dolor-quibusdam/issues/48) (various 0-depth issues, bad "is output start" logic)

#### Version 2.0.0 (breaking)

-   New: automatic entity escaping for `&`, `<`, and `>` characters. In addition, quotes `"` in attributes are also escaped (see [#41](/../../issues/41)). Prior to this, users [had to provide their own filter manually](https://github.com/wemnyelezxnpm/expedita-dolor-quibusdam/issues/4#issuecomment-19165730). Note that `@wemnyelezxnpm/expedita-dolor-quibusdam` makes an effort not to escape entities that appear to have already been encoded, to prevent double-encoding issues.
...
First you'll want to require @wemnyelezxnpm/expedita-dolor-quibusdam in your script, and assign the result to the namespace variable you want to use (in this case @wemnyelezxnpm/expedita-dolor-quibusdam):

```javascript
// Node
const { toXML } = require('@wemnyelezxnpm/expedita-dolor-quibusdam');

// Browser (with the help of something like Webpack or Rollup)
import { toXML } from '@wemnyelezxnpm/expedita-dolor-quibusdam';

// Browser global fallback (requires no bundler)
var toXML = window.@wemnyelezxnpm/expedita-dolor-quibusdam.toXML;
```
